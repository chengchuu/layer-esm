...
export const createLoadingContent = (doc: Document, icon: number, text: string): HTMLDivElement => {
  const wrapper = createElement(doc, "div", [ `${PREFIX}__loading-shell` ]);
  const spinner = createElement(doc, "span", [ `${PREFIX}__spinner`, `${PREFIX}__spinner--${Math.max(icon, 0)}` ]);
...
export const createTipBubble = (doc: Document, html: string, direction: LayerTipDirection): HTMLDivElement => {
  const bubble = createElement(doc, "div", [ `${PREFIX}__tips` ]);
  appendHTML(bubble, html);

  const arrow = createElement(doc, "span", [ `${PREFIX}__tip-arrow`, `${PREFIX}__tip-arrow--${direction}` ]);
