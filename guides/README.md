# Release Notes

- [layer-esm v1.2.3: 消息提示、对话框与图标](RELEASE_NOTES/introducing-layer-esm-v1.2.3.zh-CN.md)
- [layer-esm v1.0.1: Layer-style dialogs for modern web projects](https://github.com/chengchuu/layer-esm/blob/main/guides/RELEASE_NOTES/introducing-layer-esm-v1.0.1.en-US.md)
- [layer-esm v1.0.1：面向现代 Web 项目的 Layer 风格弹层库](https://github.com/chengchuu/layer-esm/blob/main/guides/RELEASE_NOTES/introducing-layer-esm-v1.0.1.zh-CN.md)
- [Change Record](https://github.com/chengchuu/layer-esm/blob/main/guides/CHANGES.md)
- [Publish Log](https://github.com/chengchuu/layer-esm/blob/main/guides/PUBLISH_LOG.md)
- [Third-Party Notices](https://github.com/chengchuu/layer-esm/blob/main/guides/THIRD_PARTY_NOTICES.md)

The guides directory records repository-level change notes, publish logs,
third-party notices, and longer-form articles for `layer-esm`.
